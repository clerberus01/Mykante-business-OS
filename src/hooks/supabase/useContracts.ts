...
import type { Contract, TemplateMarketplaceItem } from '../../types';
...
export function useSupabaseContracts() {
  ...
  const contractsQueryKey = useMemo(() => ['contracts', organizationId] as const, [organizationId]);
  const marketplaceQueryKey = useMemo(() => ['template-marketplace', organizationId] as const, [organizationId]);
