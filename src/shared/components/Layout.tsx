...
const NavItem: React.FC<NavItemProps> = ({ icon: Icon, label, id, to, active, onClick }) => (
  <button
    id={`nav-${id}`}
    onClick={() => onClick(to)}
    title={label}
...
const navItems = [
  { id: 'dashboard', label: 'Painel', icon: LayoutDashboard, to: '/' },
  { id: 'crm', label: 'CRM & Timeline', icon: Users, to: '/crm' },
  { id: 'projects', label: 'Projetos', icon: Briefcase, to: '/projects' },
  { id: 'calendar', label: 'Calendario', icon: Calendar, to: '/calendar' },
  { id: 'finance', label: 'Financeiro', icon: Wallet, to: '/finance' },
  { id: 'messages', label: 'Comunicacao', icon: MessageSquare, to: '/messages' },
  { id: 'docs', label: 'Documentos', icon: FileText, to: '/docs' },
  { id: 'contracts', label: 'Contratos', icon: FileSignature, to: '/contracts' },
  { id: 'automations', label: 'Automations', icon: Workflow, to: '/automations' },
];

const routeNavIds: Record<string, string> = Object.fromEntries(navItems.map((item) => [item.to, item.id]));
routeNavIds['/settings'] = 'settings';
routeNavIds['/admin'] = 'platform-admin';

function useAppNavigation() {
  const navigate = useNavigate();
  const pathname = useRouterState({ select: (state) => state.location.pathname });
  const activeNavId = routeNavIds[pathname] ?? 'dashboard';
  const routeKey = activeNavId;
  ...
  return { activeNavId, goTo, navigate, routeKey };
}
...
const Header = () => {
  ...
  const searchResults = React.useMemo(() => {
    ...
    const projectResults = projects
      ...
      .map((project) => ({
        ...
        subtitle: 'Projeto',
        action: () => {
          void navigate({ to: '/projects', search: { projectId: project.id } });
        },
      }));
    ...
    const clientResults = clients
      ...
      .map((client) => ({
        ...
        subtitle: 'Cliente',
        action: () => {
          void navigate({ to: '/crm', search: { clientId: client.id } });
        },
      }));
  ...
  }, [clients, documents, navigate, normalizedQuery, projects]);
  ...
  const quickActions = [
    {
      ...
      label: 'Novo Cliente',
      action: () => {
        void navigate({ to: '/crm', search: { action: 'create-client' } });
      },
    },
    {
      id: 'create-project',
      label: 'Novo Projeto',
      action: () => {
        void navigate({ to: '/projects', search: { action: 'create-project' } });
      },
    },
    {
      ...
      action: () => {
        void navigate({ to: '/finance', search: { action: 'create-transaction' } });
      },
    },
    {
      id: 'upload-document',
      ...
      action: () => {
        void navigate({ to: '/docs', search: { action: 'upload-document' } });
      },
    },
  ];
...
export default function Layout({
  ...
  const { routeKey } = useAppNavigation();
  useDomainEventsRealtime();
