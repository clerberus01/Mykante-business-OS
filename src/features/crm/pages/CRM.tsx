...
export default function CRM() {
  ...
  const routeSearch = useRouterState({
    select: (state) => state.location.search as { clientId?: string; action?: string },
  });
  ...
  React.useEffect(() => {
    if (routeSearch.clientId) {
      setSelectedClientId(routeSearch.clientId);
      void navigate({ to: '/crm', search: {}, replace: true });
      return;
    }

    if (routeSearch.action === 'create-client') {
  ...
  }, [navigate, routeSearch.action, routeSearch.clientId]);
