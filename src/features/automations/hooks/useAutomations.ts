...
import type { AutomationRule, AutomationRun } from '@/src/types';
...
export function useSupabaseAutomations() {
  ...
  return {
    ...
    refreshAutomations: async () => {
      await Promise.all([
        queryClient.fetchQuery({ queryKey: rulesQueryKey, queryFn: loadRules }),
        queryClient.fetchQuery({ queryKey: runsQueryKey, queryFn: loadRuns }),
      ]);
    },
  };
}
