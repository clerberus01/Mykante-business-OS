...
export default function Finance() {
  ...
  const routeSearch = useRouterState({
    select: (state) => state.location.search as { action?: string; timestamp?: number },
  });
  const {
    transactions,
    categories,
  ...
  React.useEffect(() => {
    if (routeSearch.action === 'create-transaction') {
      setEditingTransaction(undefined);
      setDraftTimestamp(routeSearch.timestamp);
      setShowModal(true);
      void navigate({ to: '/finance', search: {}, replace: true });
      return;
    }
  ...
  }, [navigate, routeSearch.action, routeSearch.timestamp]);
